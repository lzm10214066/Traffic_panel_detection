...
LBP & LBP::calcLBP( Mat d_img, double radius, bool borderCopy ) {
	...
	vector<vector<double> > spoints(samples);
...
/** ******************************************************************
*
* Descriptor part CLBP_M
*
*/
LBP & LBP::calcLBP_M(Mat d_img, double radius, bool borderCopy) {
	...
	if (borderCopy) {
		Mat tmp(d_img.rows + 2 * radius, d_img.cols + 2 * radius, CV_64F);
		copyMakeBorder(d_img, tmp, radius, radius, radius, radius, BORDER_REPLICATE, Scalar(0));
		d_img = tmp.clone();
	}
	...
	//	startTime = clock();
	double meanM = 0;
	vector<Mat> diffM(samples);
	for (int i = 0; i < samples; ++i)
	{
		diffM[i].create(dy, dx, CV_64FC1);
	}
	...
	for (int i = 0; i < samples; i++) 
	{
		...
		if ((fabs(x - rx) < 1e-6) && (fabs(y - ry) < 1e-6))
		{
			...
			Mat N(d_img, Rect(rx, ry, dx, dy));
			//compare(N, d_C, D, CMP_GE); // D = (N >= C);
			absdiff(N, d_C, diffM[i]); //D = abs(N - C);
		}
		else 
		{
			// Interpolation needed, use double type images
		...
		double mean_i = mean(diffM[i])[0];
		meanM += mean_i;
	}

	double thres = meanM / samples;
	Mat c(dy, dx, CV_64FC1, thres);
	for (int i = 0; i < samples; ++i)
	{
		Mat dm = diffM[i];
		compare(dm,c, D, CMP_GE); // D = (N >= C);
		// Update the result matrix.
		double v = pow(2., i) / 255.; // Divide by 255 because D is 0/255 rather than 0/1
		D.convertTo(D, CV_64F);
		result = result + (v * D);
